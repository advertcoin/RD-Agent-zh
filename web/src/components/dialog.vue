<template>
  <div class="dialog-box" v-if="uniShowDialog">
    <div class="dialog-content gradient-border">
      <h1>{{ t('dialog.increase_loop') }}</h1>
      <p>{{ t('dialog.increase_loop_desc') }}</p>
      <el-radio-group v-model="radio1">
        <el-radio :value="'5'">5 {{ t('dialog.loops') }}</el-radio>
        <el-radio :value="'10'">10 {{ t('dialog.loops') }}</el-radio>
        <el-radio :value="'20'">20 {{ t('dialog.loops') }}</el-radio>
        <el-radio value="num"
          ><el-input-number
            class="number-input"
            v-model="num"
            ...
            @change="handleChange"
          />
          {{ t('dialog.loops') }}</el-radio
        >
      </el-radio-group>
      <div class="btn-box">
        <button class="gradient-border back" @click="close">{{ t('dialog.back') }}</button>
        <button class="add-loops active">{{ t('dialog.add_loops') }}</button>
      </div>
    </div>
  </div>
</template>
